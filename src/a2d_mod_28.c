#include "cstd.h"
static float c[] = {-1 / 560, 8 / 315, -1 / 5, 8 / 5, -205 / 72, 8 / 5, -1 / 5, 8 / 315, -1 / 560};
float **eal2d(sim *s);
    ...
    for (int it = 0; it < nt; it++)
    {
        for (int ix = 4; ix < nxb - 4; ix++)
        {
            for (int iz = 4; iz < nzb - 4; iz++)
            {
                ...
                p2[ix][iz] = (2 - SQUARE(d[ix][iz] * dt)) / alpha * p1[ix][iz] + (d[ix][iz] * dt - 1) / alpha * p0[ix][iz] + SQUARE(dt * v[ix][iz]) / alpha * lap;
            }
        }
    ...
    for (int it = 0; it < nt; it++)
    {
        for (int ix = 4; ix < nxb - 4; ix++)
        {
            for (int iz = 4; iz < nzb - 4; iz++)
            {
                ...
                lap = lap_x / SQUARE(dx) + lap_z / SQUARE(dz);
                p2[ix][iz] = 2 * p1[ix][iz] - p0[ix][iz] + SQUARE(v[ix][iz] * dt) * lap;
                p2[ix][iz] *= d[ix][iz];
            }
        }
        tmp = p0, p0 = p1, p1 = p2, p2 = tmp;

...
void free4 (void ****p);
void *****alloc5 (size_t n1, size_t n2, size_t n3, size_t n4, size_t n5, size_t
size); void free5 (void *****p); void ******alloc6 (size_t n1, size_t n2, size_t
n3, size_t n4, size_t n5, size_t n6, size_t size); void free6 (void ******p);
...
void ****alloc4(size_t n1, size_t n2, size_t n3, size_t n4, size_t size) {
    ...
    for (i4 = 0; i4 < n4; i4++) {
        ...
        for (i3 = 0; i3 < n3; i3++) {
            ...
            for (i2 = 0; i2 < n2; i2++)
                p[i4][i3][i2] =
                    (char *)p[0][0][0] + size * n1 * (i2 + n2 * (i3 + n3 * i4));
        }
    }
    return p;
}
...
void *****alloc5(size_t n1, size_t n2, size_t n3, size_t n4, size_t n5,
    ...
    if ((p[0][0][0] = (void **)malloc(n5 * n4 * n3 * n2 * sizeof(void *))) ==
        NULL) {
        free(p[0][0]);
        free(p[0]);
        free(p);
    ...
    if ((p[0][0][0][0] = (void *)malloc(n5 * n4 * n3 * n2 * n1 * size)) ==
        NULL) {
        free(p[0][0][0]);
        free(p[0][0]);
        free(p[0]);
...
void ******alloc6(size_t n1, size_t n2, size_t n3, size_t n4, size_t n5,
    ...
    if ((p[0][0] = (void ****)malloc(n6 * n5 * n4 * sizeof(void ***))) ==
        NULL) {
        free(p[0]);
        free(p);
        err("%s : alloc6: malloc failed", __FILE__);
    }
    if ((p[0][0][0] = (void ***)malloc(n6 * n5 * n4 * n3 * sizeof(void **))) ==
        NULL) {
        free(p[0][0]);
        free(p[0]);
        free(p);
    ...
    if ((p[0][0][0][0] = (void **)malloc(n6 * n5 * n4 * n3 * n2 *
                                         sizeof(void *))) == NULL) {
        free(p[0][0][0]);
        free(p[0][0]);
        free(p[0]);
    ...
    if ((p[0][0][0][0][0] =
             (void *)malloc(n6 * n5 * n4 * n3 * n2 * n1 * size)) == NULL) {
        free(p[0][0][0][0]);
        free(p[0][0][0]);
        free(p[0][0]);
        free(p[0]);
...
/* allocate a 1-d array of ints */
int *alloc1int(size_t n1) { return (int *)alloc1(n1, sizeof(int)); }
...
void free2int(int **p) { free2((void **)p); }
...
void free3int(int ***p) { free3((void ***)p); }

/* allocate a 1-d array of floats */
float *alloc1float(size_t n1) { return (float *)alloc1(n1, sizeof(float)); }
...
void free1float(float *p) { free1(p); }
...
void free2float(float **p) { free2((void **)p); }
...
void free3float(float ***p) { free3((void ***)p); }
...
void free4float(float ****p) { free4((void ****)p); }
...
void free5float(float *****p) { free5((void *****)p); }
...
void free6float(float ******p) { free6((void ******)p); }
...
void free4int(int ****p) { free4((void ****)p); }
...
void free5int(int *****p) { free5((void *****)p); }

/* allocate a 5-d array of chars, added by Zhaobo Meng, 1997 */
unsigned char *****alloc5uchar(size_t n1, size_t n2, size_t n3, size_t n4,
                               size_t n5) {
    return (unsigned char *****)alloc5(n1, n2, n3, n4, n5,
                                       sizeof(unsigned char));
}

/* free a 5-d array of chars, added by Zhaobo Meng, 1997 */
void free5uchar(unsigned char *****p) { free5((void *****)p); }

/* allocate a 5-d array of ints, added by Zhaobo Meng, 1997 */
unsigned short *****alloc5ushort(size_t n1, size_t n2, size_t n3, size_t n4,
                                 size_t n5) {
    return (unsigned short *****)alloc5(n1, n2, n3, n4, n5,
                                        sizeof(unsigned short));
}

/* free a 5-d array of shorts, added by Zhaobo Meng, 1997 */
void free5ushort(unsigned short *****p) { free5((void *****)p); }

/* allocate a 6-d array of ints, added by Zhaobo Meng, 1997 */
unsigned short ******alloc6ushort(size_t n1, size_t n2, size_t n3, size_t n4,
                                  size_t n5, size_t n6) {
    return (unsigned short ******)alloc6(n1, n2, n3, n4, n5, n6,
                                         sizeof(unsigned short));
}

/* free a 6-d array of shorts, added by Zhaobo Meng, 1997 */
void free6ushort(unsigned short ******p) { free6((void ******)p); }

/* allocate a 1-d array of doubles */
double *alloc1double(size_t n1) { return (double *)alloc1(n1, sizeof(double)); }
...
void free1double(double *p) { free1(p); }
...
void free2double(double **p) { free2((void **)p); }
...
void free3double(double ***p) { free3((void ***)p); }

/* allocate a 1-d array of cpxs */
cpx *alloc1cpx(size_t n1) { return (cpx *)alloc1(n1, sizeof(cpx)); }
...
void free2cpx(cpx **p) { free2((void **)p); }
...
void free3cpx(cpx ***p) { free3((void ***)p); }

/*my code starts here to alloc double cpx functions*/

/* allocate a 1-d array of cpxs */
zpx *alloc1zpx(size_t n1) { return (zpx *)alloc1(n1, sizeof(zpx)); }
...
/* free a 2-d array of cpxs */
void free2zpx(zpx **p) { free2((void **)p); }
...
/* free a 3-d array of cpxs */
void free3zpx(zpx ***p) { free3((void ***)p); }
/* added by Guo Jiadong add,2025 */
char *alloc1char(size_t n1) { return (char *)alloc1(n1, sizeof(char)); }
char *realloc1char(char *v, size_t n1) {
    return (char *)realloc1(v, n1, sizeof(char));
}
void free1char(char *p) { free1(p); }
/* Copyright (c) Colorado School of Mines, 2011.*/
/* All rights reserved.                       */
...
size_t efread(void *bufptr, size_t size, size_t count, FILE *stream) {
    ...
    if (nread != count && ferror(stream))
        err("%s: efread: fread only %d items of %d", __FILE__, nread, count);
...
int getparint(char *name, int *ptr) { return getnpar(0, name, "i", ptr); }
int getparuint(char *name, unsigned int *ptr) {
    return getnpar(0, name, "p", ptr);
}
int getparshort(char *name, short *ptr) { return getnpar(0, name, "h", ptr); }
int getparushort(char *name, unsigned short *ptr) {
    return getnpar(0, name, "u", ptr);
}
int getparlong(char *name, long *ptr) { return getnpar(0, name, "l", ptr); }
int getparulong(char *name, unsigned long *ptr) {
    return getnpar(0, name, "v", ptr);
}
int getparfloat(char *name, float *ptr) { return getnpar(0, name, "f", ptr); }
int getpardouble(char *name, double *ptr) { return getnpar(0, name, "d", ptr); }
int getparstring(char *name, char **ptr) { return getnpar(0, name, "s", ptr); }
...
int getnpar(int n, char *name, char *type, void *ptr) {
    ...
    for (nval = 0, aval = argtbl[i].asciival; *aval; nval++) {
        ...
        default:
            fprintf(stderr, "%s: invalid parameter type = %s", __FILE__, type);
        }
        while (*aval++ != ',') {
            if (!*aval)
                break;
...
int countparval(char *name) { return countnparval(0, name); }
/*
            ...
            *eqptr = (char)0;
            ...
            if (debug) {
                fprintf(stderr, "argtbl[%d]: name=%s asciival=%s\n", nargs,
                        argtbl[nargs].name, argtbl[nargs].asciival);
            }
            nargs++;
        }
    }
